...
<main>
    <section class="container dms-cart-page">
        <form action="{{ route('retailerplaceorder') }}" method="POST" enctype="multipart/form-data" id="orderForm">
            @csrf
            <input type="hidden" name="weight" id="hdweight" value="{{ $cartWeight }}">
            @foreach ($cart as $item)
            <input type="hidden" name="moq{{ $item->id }}" id="moq{{ $item->product_id }}" value="{{ $item->moq }}">
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-10">
                                    <input type="text" class="form-control" name="box{{ $item->id }}"
                                        id="box{{ $item->id }}" value="{{ $item->box != null ? $item->box : '' }}">
                                </div>
                            </div>
                            <div>
                                <div>
                                ...
                                <div>
                                    <textarea style="height: 150px;" id="others{{ $item->id }}"
                                        name="others{{ $item->id }}" maxlength="255"
                                        class="form-control">{{ $item->others != null ? $item->others : '' }}</textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-warning" onclick="saveRemarks({{ $item->id }})"
            ...
            <div id="cart-no-items" class="py-4">
                <div class="text-center">
                    ...
                    <div>
                        <img class="img-fluid" width="212" height="212" src="{{ asset('cart-no-tems.gif') }}" alt="">
                    </div>
                ...
                <div class="col-12 col-xl-9 col-xxl-9  position-relative mb-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12 col-lg-4">
                            ...
                            <input placeholder="Reference" type="text" class="form-control" name="reference"
                                id="reference" @if (Auth::user()->role_id == App\Enums\Roles::CRM) required @endif>
                    ...
                    <div class="card cart-page_total-card mb-4">
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="mt-3 mb-3 form-control" maxlength="255" name="others" id="others"
...
<script src="{{ asset('retailer/assets/js/cart/cart.js') }}"></script>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (isDesktopView) {
        linkElement.href = "{{ route('retailerlanding') }}";
    } else {
        linkElement.href = "{{ route('retailercategory') }}";
    }
});
</script>
<script>
