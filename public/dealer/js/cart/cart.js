            ...
            <div class="text-center">
                ...
                response.carts.forEach((item) => {
                    ...
                    <td class="text-center">${item.style_name}</td>
                                            <td class="text-center">
                         <select class="badge cart-items_card_finish-badge" name="finish${
                             item.id
                         }" id="finish" data-id="${
                        item.id
                    }" style="width: 100%;">
                         <option value="">Select Finish Name</option>
                         ${generateFinishOptions(
                             response.finishes[item.project_id] || [],
                             item.finish_id
                         )}</select> 
                    </td>
                        <td class="text-center">
                ...
                response.carts.forEach((item, index) => {
                    ...
                    <div class="d-flex">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="cart-page_card-light-text mb-2">Size(cm): ${
                                item.height && item.width
                        ...
                        <div class="mb-2">
                            <select class="badge cart-items_card_finish-badge w-75" name="finish${
                                item.id
                            }" id="finish" data-id="${
                        item.id
                    }" style="width:100%">
...
// function generateFinishOptions(finishes, selectedFinish) {
//     if (Array.isArray(finishes)) {
//         return finishes
//             .map(
//                 (finish) =>
//                     `<option value="${finish.id}" ${
//                         finish.id === selectedFinish ? "selected" : ""
//                     }>${finish.finish_name}</option>`
//             )
//             .join("");
//     } else {
//         console.error("Finishes is not an array:", finishes);
//         return "";
//     }
// }
...
$(document).on("change", "#finish", function (event) {
    let cartId = $(this).data("id");
    let finishId = event.target.value;
    saveFinishId(finishId, cartId);
});

function saveFinishId(finishId, cartId) {
    // Send an AJAX request to your server to save the finish_id
    $.ajax({
        url: "addfinish",
        method: "GET",
        data: {
        ...
        success: function (response) {
            toastr.success("Finish Updated Successfully", "", {
                progressBar: true,
                closeButton: true,
            });
        },
        error: function (xhr, status, error) {
            console.error("Error saving finish ID:", error);
        },
    });
}
