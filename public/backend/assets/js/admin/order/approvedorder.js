...
function format(d) {
    // Sort products so that approved ones come first
    d.products.sort((a, b) => b.is_approved - a.is_approved);
        ...
        .map((product) => {
            ...
            let badgeClass =
                availableQuantity < totalQuantity
                    ...
                    : "badge-success";
            let rowStyle =
                product.is_approved === 0 ? "style='opacity:30%;'" : "";
    ...
    return `<table cellpadding="5" cellspacing="0" border="0" class="table" style="padding-left:50px;" width="100%">
                ...
                <div class="d-flex flex-wrap gap-3 px-5 pt-2 border-1 border-bottom">
                <div class="d-flex flex-column  align-items-center  mr-3">
                        <label for="" class="mr-2 mb-1" style="font-weight: 700;color: #000;">Select Dealer</label>
                  <input class="form-control mb-4" name="dealer" id="dealer" value="${
                      d.dealer_name
                  }">
                </div>
...
$(document).ready(function () {
    ...
    $("#approvedorder tbody").on("click", "td.dt-control", function () {
        let tr = $(this).closest("tr");
        let row = table.row(tr);

        // Check if any row is already shown
        let shownRows = $("#approvedorder tbody tr.shown");
