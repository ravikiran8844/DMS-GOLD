$(document).ready(function () {
    $("#tableZone").DataTable({
        processing: true,
        serverSide: true,
        // responsive: true,
        order: [
            [0, "ASC"]
        ],
        ajax: "getzonedata",
        fnRowCallback: serialNoCount,
        createdRow: function (row, data, dataIndex) {
            // Add tooltip to the 'Edit' icon in the 'Action' column (column index 5)
            $('td:eq(3) .edit', row).attr('data-bs-toggle', 'tooltip').attr('data-bs-placement', 'top').attr('title', 'Edit');

            // Add tooltip to the 'Delete' icon in the 'Action' column (column index 5)
            $('td:eq(3) .delete', row).attr('data-bs-toggle', 'tooltip').attr('data-bs-placement', 'top').attr('title', 'Delete');

            // Initialize tooltips for the created row
            $('[data-bs-toggle="tooltip"]', row).tooltip();
        },
        columns: [{
                data: "id"
            },
            {
                data: "zone_name"
            },
            {
                data: "is_active",
                orderable: false,
                searchable: false,
                render: function (data, type, row) {
                    return `<div class="pretty p-switch p-fill">
                    <input type="checkbox" id="chkZone${row.id}" 
                    onclick="doStatus(${row.id});" ${data == 1 ? "checked" : ""}>
                    <div class="state p-success">
                        <label></label>
                    </div>
                </div>`;
                },
            },
            {
                data: "action",
                orderable: false,
                searchable: false
            },
        ],
    });
});

function cancels() {
    $("#zone_name").val("");
    $("#save").text("Save");
    $("#heading").text("Add Zone");
}

function doEdit(id) {
    $("#zoneId").val(id);
    $("#zone_name").focus();
    $("#save").text("Update");
    $("#heading").text("Update Zone");
    getZoneById(id);
}

function getZoneById(id) {
    $.ajax({
        type: "GET",
        url: "getzone/" + id,
        dataType: "json",
        success: function (data) {
            $("#zone_name").val(data.zone.zone_name);
        },
    });
}

function showDelete(id) {
    confirmDelete(id, "deletezone/", "tableZone");
}

function doStatus(id) {
    var status = $("#chkZone" + id).is(":checked");
    confirmStatusChange(
        id,
        "zonestatus/",
        "tableZone",
        status == true ? 1 : 0,
        "chkZone"
    );
}
